...
def main(config_path=None):
    ...
    print(f"[params] neurons/col={int(mp.N.sum())} internal synapses={int(mp.n_syn.sum()):,} "
    ...
    print("[rates] " + " ".join(f"{k}:{v:.1f}" for k, v in pop_rates.items()))
    print("[bands] " + " ".join(f"{b}:{bp[b]:.2f}" for b in bands))
    print(f"[plv] theta-beta {nm[0]}:{nm[1]} diag(mean)={np.mean(np.diag(plv_M)):.3f} "
